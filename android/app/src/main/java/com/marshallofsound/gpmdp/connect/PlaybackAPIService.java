...
public class PlaybackAPIService extends Service implements AudioManager.OnAudioFocusChangeListener {
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        if (intent != null && intent.hasExtra("command") && ws != null) {
        ...
        }  else if (intent != null && intent.hasExtra("email")) {
            if (t != null) {
            ...
            t = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    Log.d(WS_TAG, "Connecting for " + email);
                    if (ws != null) {
                        ws.disconnect();
                        ws = null;
                    }
                    try {
                        ws = factory.createSocket("ws://connect.gpmdp.xyz");
                        ...
                        Log.d(WS_TAG, "FAILED TO CONNECT TO " + email);
                        if (ws != null) {
                            ws.disconnect();
                        }
                        ws = null;
                    ...
                    if (ws != null) {
                        Log.d(WS_TAG, "CONNECTED TO " + email);
                    } else {
